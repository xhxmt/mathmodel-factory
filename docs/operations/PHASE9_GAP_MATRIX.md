...
| A2_0016-A2_0020 migrations | Additive fixed-offline implementation; earlier published statement bytes retained | Packaged migration source/fresh logs exercise prefix/checksum, upgrade, interruption and restore behavior, including Authority-backed P0 runner and replay/runtime evidence attestation | All five `NOT APPLIED` in production | Explicit migration approval, verified backup and durable journal |
...
| Actual formal runtime coordination | Implemented candidate; independent review pending | A2_0020 plan/execute/collect/export/finalize entry, durable pre-dispatch identities, actual OS observations and normal runtime receipt writer calls | Not executed against production Authority | Exact candidate audit, migration approval, real Authority/input coordinate and separate runtime grant; tests are not formal receipts |
...
external gates. Runtime coordination must pass current-candidate validation and independent
review before production use. No fixture,
