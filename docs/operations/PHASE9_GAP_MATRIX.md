...
| Audit command/log/summary closure | Fixed-offline policy and tooling | Package requires seven source/fresh pairs (14 records plus 14 complete raw logs), a derived exact-match summary and append-only failed attempts; builder verification is first-party | Independent audit `PENDING` | Independent verifier recomputes every record, log, outcome and ZIP closure |
