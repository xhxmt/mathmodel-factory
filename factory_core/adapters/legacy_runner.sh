...
# ─────────────────────────────────────────────────────────────────────
# legacy_runner.sh — frozen step implementation adapter
#
...
# actual progress from file state for unmigrated projects. Migrated projects
# invoke this file for one step and one attempt through FactoryEngine.
#
# Usage:
...
if [[ "${FACTORY_ENGINE_SINGLE_STEP:-0}" == "1" ]]; then
    MAX_RETRIES=1
    MAX_RETRIES_STEP6=1
    MAX_RETRIES_STEP13=1
fi
...
if [[ -z "${RUN_PAPER_SNAPSHOT:-}" ]]; then
    ...
    if cp "${BASH_SOURCE[0]}" "$SNAPSHOT_PATH"; then
...
if [[ "${FACTORY_ENGINE_SINGLE_STEP:-0}" != "1" && -f "$FACTORY/scripts/state_manager.sh" ]]; then
...
if [[ "${FACTORY_ENGINE_SINGLE_STEP:-0}" == "1" ]]; then
    EXPECTED_STEP="${FACTORY_ENGINE_STEP_ID:?FACTORY_ENGINE_STEP_ID is required}"
    if (( EXPECTED_STEP != STEP + 1 )); then
        log "ERROR: engine requested Step $EXPECTED_STEP but legacy artifacts infer next Step $((STEP + 1))"
        exit 64
    fi
fi
...
if (( STEP < 0 )); then
    ...
    if [[ "${FACTORY_MODELING_ONLY:-0}" == "1" ]] || is_modeling_input "$PROJECT" "$QUESTION"; then
...
if [[ "${FACTORY_ENGINE_SINGLE_STEP:-0}" == "1" && "$STEP" == "0" ]]; then
    exit 0
fi
...
while (( STEP < 16 )); do
    ...
    while (( RETRIES < STEP_MAX_RETRIES )); do
        ...
        if (( NEXT == 16 && STEP_RC == 43 )); then
            ...
            [[ "${FACTORY_ENGINE_SINGLE_STEP:-0}" == "1" ]] && exit 75
            break
        fi
        ...
        if verify_step "$NEXT" && verify_step_output "$NEXT"; then
            ...
            _set_checkpoint_step "$STEP"
            [[ "${FACTORY_ENGINE_SINGLE_STEP:-0}" == "1" ]] && exit 0
            break
        else
            RETRIES=$((RETRIES + 1))
