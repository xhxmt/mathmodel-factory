...
def artifact_pattern_variants(pattern: str) -> tuple[str, ...]:
    """Return the deterministic v1 globstar variants for one owner pattern."""
...
def artifact_pattern_matches(pattern: str, path: str) -> bool:
    """Apply the case-insensitive, normalized v1 owner matching semantics."""

    normalized = normalize_artifact_path(path).lower()
    return any(
        fnmatchcase(normalized, candidate)
        for candidate in artifact_pattern_variants(pattern)
    )


def artifact_ownership(path: str) -> ArtifactOwnership | None:
    for ownership in ARTIFACT_OWNERSHIP_REGISTRY:
        if artifact_pattern_matches(ownership.pattern, path):
            return ownership
    return None
