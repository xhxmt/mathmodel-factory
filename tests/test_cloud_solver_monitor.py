...
def test_health_check_uses_identity_token_without_recording_it(monkeypatch):
    seen = {}
    monkeypatch.setattr(monitor, "get_identity_token", lambda: "sensitive-id-token")
...
def test_health_check_classifies_cloud_run_auth_failure(monkeypatch):
    monkeypatch.setattr(monitor, "get_identity_token", lambda: "id-token")
    monkeypatch.setattr(
        monitor.requests,
        "get",
    ...
    assert result["error"] == "Cloud Run authentication failed (HTTP 403)"
